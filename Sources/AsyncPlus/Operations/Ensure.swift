...
// Note: There is no point using `ensure` with a non-failable chained item (you might as well use `then`). There is also not much of a use case for having an `ensure` operation with a failable body, as it is unclear what the desired behavior here would be for passing on an error.
...
extension ChainableResult {
    ...
    internal func ensureAsyncBodyWithResultType<PromiseType: ChainablePromise<T>>(_ body: @escaping () async -> (), resultType: PromiseType.Type) -> PromiseType {
